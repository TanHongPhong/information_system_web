-- Migration: Fix vehicle filter by origin_region - Mở rộng status và cải thiện logic
-- Mục đích: Hiển thị tất cả xe ở vùng xuất phát, không chỉ xe AVAILABLE
...
BEGIN
  ...
  WHERE v.company_id = p_company_id
    -- MỞ RỘNG: Lấy cả AVAILABLE và IN_USE (nếu không có orders active)
    -- Loại bỏ MAINTENANCE và các status không hợp lệ
    ...
    -- Nếu xe có status IN_USE, kiểm tra xem có đơn hàng active không
    -- Nếu có đơn hàng LOADING, IN_TRANSIT, COMPLETED thì không hiển thị
    AND NOT EXISTS(
...
Function này sẽ:
- Lấy xe có status AVAILABLE hoặc IN_USE (nếu không có orders active)
- Xe phải ở vùng xuất phát (origin_region) để có thể bốc hàng
- Xe phải có route từ origin_region đến destination_region
- Loại bỏ xe đang có đơn hàng với status LOADING, IN_TRANSIT, COMPLETED';
