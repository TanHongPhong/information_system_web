import { Cpu, Map, Shield, Headphones } from "./Icons";
  ...
  return (
    <section id="features" className="py-12 lg:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-2xl md:text-3xl font-extrabold title-grad">
          Vì sao chọn <span className="text-blue-700">6A Logistics</span>?
        </h2>
        <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {items.map(({ Icon, title, desc }) => (
            <div key={title} className="p-6 rounded-2xl bg-gradient-to-b from-white to-brand-50 border border-brand-100 hover:shadow-[0_10px_24px_rgba(37,99,235,.32)] transition">
