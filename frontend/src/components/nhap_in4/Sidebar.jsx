...
const IconBtn = ({ active, title, children }) => (
  <button
    title={title}
    className={[
  ...
  return (
    <aside className="fixed inset-y-0 left-0 w-20 bg-white border-r border-slate-200 flex flex-col items-center gap-3 p-3">
      ...
      <div className="flex flex-col items-center gap-4">
        <IconBtn title="Trang chủ" active><Home /></IconBtn>
        <IconBtn title="Theo dõi vị trí"><Map /></IconBtn>
        <IconBtn title="Lịch sử giao dịch"><FileText /></IconBtn>
        <IconBtn title="Người dùng"><User /></IconBtn>
        <IconBtn title="Cài đặt"><Settings /></IconBtn>
      </div>
    </aside>
  );
}
