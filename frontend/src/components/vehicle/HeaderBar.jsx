...
/* Ô tìm kiếm kiểu pill: viền gradient xanh rất nhẹ + đổ sáng mềm */
function SearchField({ value, onChange, placeholder }) {
  return (
    <div className="group relative">
      {/* viền gradient + ánh sáng nền rất nhẹ */}
      <div className="rounded-full p-[3px] bg-gradient-to-r from-sky-100 via-sky-50 to-sky-100
                      shadow-[0_2px_10px_rgba(59,130,246,.12)] transition
                      group-focus-within:shadow-[0_2px_14px_rgba(59,130,246,.18)]">
        {/* lõi trắng bên trong */}
        <div className="relative rounded-full bg-white border border-sky-100">
          ...
          <input
            ...
            className="w-full h-12 pl-10 pr-14 rounded-full bg-transparent outline-none border-0
                       placeholder:text-slate-400 text-[15px]"
          />
          {/* nút filter nhỏ ở trong ô */}
          <button
            type="button"
            title="Bộ lọc"
            className="absolute right-1.5 top-1/2 -translate-y-1/2 h-8 w-8 grid place-items-center
                       rounded-xl bg-white border border-slate-200 hover:bg-slate-50"
          >
...
export default function HeaderBar({ keyword, onKeywordChange, placeholder = "Tìm tuyến đường, loại xe, tài xế..." }) {
  return (
